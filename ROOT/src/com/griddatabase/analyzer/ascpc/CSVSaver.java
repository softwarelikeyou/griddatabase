...
import com.griddatabase.pojo.ASCPCCSV;
...
public class CSVSaver {
	...
	public boolean execute() {
		...
		try {
            ...
            CsvToBean<ASCPCCSV> bean = new CsvToBean<ASCPCCSV>();
            ColumnPositionMappingStrategy<ASCPCCSV> strat = new ColumnPositionMappingStrategy<ASCPCCSV>();
            strat.setType(ASCPCCSV.class);
            strat.setColumnMapping(ASCPCCSV.columns);
			List<ASCPCCSV> rows = bean.parse(strat, reader);
			Map<String, ASCPC> map = new HashMap<String, ASCPC>();
			for (ASCPCCSV row : rows)
